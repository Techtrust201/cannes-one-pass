#!/bin/bash
# Import CSV Neon → Supabase avec colonnes explicites
SUPA_URL="postgresql://postgres.avbioovvnoepogtawtzp:sI51yfH2aFeJzsX4@aws-1-eu-west-1.pooler.supabase.com:5432/postgres"
IN="/tmp/neon_export"

echo "🗑️  Vidage des tables Supabase (ordre inverse pour FK)..."
psql "$SUPA_URL" <<'TRUNCATE_SQL'
SET session_replication_role = 'replica';
TRUNCATE "ChatMessage" CASCADE;
TRUNCATE "VehicleTimeSlot" CASCADE;
TRUNCATE "AccreditationHistoryArchive" CASCADE;
TRUNCATE "AccreditationHistory" CASCADE;
TRUNCATE "AccreditationEmailHistory" CASCADE;
TRUNCATE "ZoneMovement" CASCADE;
TRUNCATE "Vehicle" CASCADE;
TRUNCATE "Accreditation" CASCADE;
TRUNCATE "ZoneConfig" CASCADE;
TRUNCATE "Event" CASCADE;
TRUNCATE "user_permission" CASCADE;
...
import_table() {
  ...
  psql "$SUPA_URL" -c "SET session_replication_role = 'replica'; \\COPY \"$TABLE\" ($COLS) FROM '$FILE' WITH (FORMAT csv, HEADER true, ENCODING 'UTF8');" 2>&1
  if [ $? -eq 0 ]; then
