...
/** Export CSV des tickets de support d'une organisation (`?espace=<slug>`). */
export async function GET(req: NextRequest) {
  ...
  const headers = [
    "Date", "Statut", "Événement", "Société", "Stand",
    "Type problème", "Identification", "E-mail", "Téléphone", "Message",
  ];
  ...
  const csv = "\uFEFF" + rows.join("\n");
  const filename = `tickets-${espace}-${new Date().toISOString().split("T")[0]}.csv`;
